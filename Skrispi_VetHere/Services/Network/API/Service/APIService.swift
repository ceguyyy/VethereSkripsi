...
class APIService: APIServiceProtocol {
    ...
    init() {
            self.baseURL = "http://10.60.62.56:8000/vet"
...
//                .map { response in
//                    DoctorModel(id:UUID(uuidString: response.data.vet_doctors.doctorId) ?? UUID() , vetId: UUID(uuidString: response.data.vet_id) ?? UUID(), specializationId: UUID(uuidString: response.data.vet_id) ?? UUID(), name: response.data.vet_doctors.doctor_name, rating: response.data.vet_doctors.rating, creadtedAt: Date(), updatedAt: Date(), image: "noimage")(
//                 
//                    )
//                }
//                .eraseToAnyPublisher()
